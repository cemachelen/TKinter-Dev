...
class FindPopup(tk.Toplevel):
    def __init__(self, master):
        ...
        self.find_button = tk.Button(
            self.button_frame, text="Find All", bg="lightgrey", fg="black", command=self.find)
        self.next_button = tk.Button(
            self.button_frame, text="Next", bg="lightgrey", fg="black", command=self.jump_to_next_match)
        self.cancel_button = tk.Button(
            self.button_frame, text="Cancel", bg="lightgrey", fg="black", command=self.cancel)
...
class Editor(tk.Tk):
    def __init__(self):
        ...
        self.KEYWORDS_1 = ["import", "as", "from",
                           "def", "try", "except", "self"]
        self.KEYWORDS_FLOW = ["if", "else", "elif", "try",
                              "except", "for", "in", "while", "return", "with"]
        self.KEYWORDS_FUNCTIONS = ["print", "list",
                                   "dict", "set", "int", "float", "str"]
        ...
        self.main_text = tk.Text(
            self, bg="white", fg="black", font=("Ubuntu Mono", self.FONT_SIZE))
        self.scrollbar = tk.Scrollbar(
            self, orient="vertical", command=self.scroll_text_and_line_numbers)
    ...
    def get_menu_coordinates(self):
        ...
        menu_y = bbox[1] + self.winfo_y() + \
            self.main_text.winfo_y() + self.FONT_SIZE + 2
    ...
    def display_autocomplete_menu(self, event=None):
        ...
        try:
            currently_typed_word = self.main_text.get(
                start + " wordstart", tk.INSERT)
        except tk.TclError:
        ...
        if currently_typed_word:
            ...
            if len(suggestions) > 0:
                ...
                self.complete_menu = tk.Menu(
                    self, tearoff=0, bg="lightgrey", fg="black")

                for word in suggestions:
                    insert_word_callback = partial(
                        self.insert_word, word=word, part=currently_typed_word, index=current_index)
                    self.complete_menu.add_command(
                        label=word, command=insert_word_callback)

                self.complete_menu.post(x, y)
                self.complete_menu.bind(
                    "<Escape>", self.destroy_autocomplete_menu)
    ...
    def tag_keywords(self, event=None, current_index=None):
        ...
        line_text = self.main_text.get(
            line_beginning, line_beginning + " lineend")
        ...
        for tag in self.main_text.tag_names():
            if tag != "sel":
                self.main_text.tag_remove(
                    tag, line_beginning, line_beginning + " lineend")
    ...
    def update_line_numbers(self):
        ...
        line_number_string = "\n".join(str(no + 1)
                                       for no in range(int(number_of_lines)))
    ...
    def next_match(self, event=None):
        try:
            ...
            self.main_text.tag_remove(
                "sel", current_target, current_target_end)
            self.main_text.tag_add(
                "findmatch", current_target, current_target_end)
        except IndexError:
            pass
