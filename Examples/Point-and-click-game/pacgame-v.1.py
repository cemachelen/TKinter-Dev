...
class Game(tk.Tk):
    def __init__(self):
        ...
        self.help_box = tk.Label(self.right_frame, textvar=self.help_var,
                                 background="black", foreground="white", padx=10, pady=20)
        self.help_box.pack(side=tk.TOP, fill=tk.X, padx=10, pady=10)

        inventory_title = tk.Label(
            self.right_frame, text="Inventory:", background="grey", foreground="white")

        inventory_space = tk.Frame(
            self.right_frame, background="lightgrey", width=300, height=320)
        inventory_space.pack_propagate(0)

        inventory_space.pack(side=tk.BOTTOM)
        inventory_title.pack(side=tk.BOTTOM, fill=tk.X)

        inventory_slot_1 = tk.Button(
            inventory_space, image=self.question_mark_image, width=50, height=50)
        inventory_slot_2 = tk.Button(
            inventory_space, image=self.question_mark_image, width=50, height=50)
        inventory_slot_3 = tk.Button(
            inventory_space, image=self.question_mark_image, width=50, height=50)
    ...
    def show_next_screen(self):
        ...
        if self.current_screen_number < len(self.game_screens):
            ...
            self.screen.create_text(
                (250, 300), text="You Win!", font=self.success_font, fill="white")
...
if __name__ == "__main__":
    ...
    scene2 = GameScreen(game, "assets/scene2.png",
                        (117, 54, 329, 412), None, "You Got the Key!")
